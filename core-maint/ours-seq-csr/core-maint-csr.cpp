...
void SeqCM::CoreMaint::Forward(node_t w){
    ...
    for (int i = graph.begin[w]; i < graph.end[w]; i++){
        node_t w2 = graph.node_idx[i];
...
int SeqCM::CoreMaint::EdgeRemove(node_t u, edge_t v) {
    ...
    //remove the edge
    //this erase is much faster. why? May because the STL has high efficiency.
    ...
    for (const node_t w: Vblack) {
        ...
        V[w].color = WHITE;
        OrderDelete(w); 
        /*OrderInsert(V[T[K-1]].pre, w);  // can do better by multi-order-insert*/
    }

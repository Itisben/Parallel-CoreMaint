...
//---------------------------------------------------------------------------
// Undirected graph
//    -- (rev_edges are removed)
//
//    -- edges are doubly stored. (i.e. neighbor-list stored at both ends)
//
//    -- edge property aceess needs one more indirection
//    e.g.
//    for(node_t u=0;u<G.num_nodes; u++) {
//       for(edge_t v_idx=G.begin[u]; v_idx < G.begin[u+1]; v_idx++) { // edges
//          node_t dest = G.node_idx[v_idx];
//          float f = edge_prop [  G.undir_edge_idx[v_idx] ];
//    }  }
//
//
//---------------------------------------------------------------------------
...
friend class gm_edge_list_graph_reader;
  ...
  public:
    ...
    edge_t* e_idx2idx;  	   // O(M) array of edge_t (a mapping sorted edge idx => original edge idx, created after semi-sorting) --> [XXX: can be removed after loading?]
    edge_t* e_rev2idx;  	   // O(M) array of edge_t (a mapping reverse edge idx => original edge idx, created after reverse edge creation)

    edge_t* e_undir_idx;  // O(M) array of edge_t . [XXX] 
    ...
    // for undirected gaphs, stored edge array size =  num_edges * 2;
    edge_t num_edges() {
        return _numEdges;
    }

    void reset_num_edge(int num) {
        _numEdges = num;
    }
    ...
    //------------------------------------------------
    // Change the graph as undirected one
    //------------------------------------------------
    // Assumption: undirected graph is 'simple'
    ...
    // for debuggin and renumbering only
    void ignore_nodekey() {
        _numeric_key.clear();

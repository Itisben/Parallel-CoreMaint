...
static inline void READNB(FILE* f, size_t size, char* destination, bool& okay);
...
#define BITS_TO_EDGE(X) ((old_flipped_format)? (X) : (saved_edge_t_size == 4) ? n32tohedge(X) : n64tohedge(X))
    ...
    printf("N = %ld, M = %ld\t", (long)N,(long)M);
...
static inline void READNB(FILE* f, size_t size, char* destination, bool& okay) {
    okay = fread(destination, sizeof(char), size, f) == size;
}
...
static inline void WRITENB(FILE* f, size_t n, const char* data, bool& okay) {
    okay = (fwrite(data, sizeof(char), n, f) == n);
}

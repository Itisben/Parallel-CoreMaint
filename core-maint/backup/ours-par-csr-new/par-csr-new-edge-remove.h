...
namespace ParCM{
    ...
    public:
        ...
        // for edge insert
        volatile int islock;     // >= 0 is locked by worker p. NONE not locked.
    ...
    * in case some of them changed. */
    ...
    public:
        ...
        * Return False: the version is changed. all elements needs to update to same version.*/
        inline void push(DATA &d) { v.push_back(d); push_heap(v.begin(), v.end(), CompareOrder());}
        ...
        inline void enqueue(DATA &d);
        inline void dequeue();
        inline void update(vector<Node> &V);
    };
    ...
    * take O(logn) time for insert*/
    class CoreMaint {
        ...
        */
        ...
        public:
        CoreMaint(size_t n, PARGRAPH &graph, vector< core_t> &core);
        ~CoreMaint();
        // global lock
        inline void OMLock();
        inline void OMUnlock();
        ...
        // for edge insert
        inline void Forward(node_t w, PRIORITY_Q &PQ, vector<node_t> &Vcolor, int p);
        inline void Backward(node_t w, QUEUE &R, int p);
        inline void DoPre(node_t u, QUEUE &R, int p);
        inline void DoAdj(node_t u, QUEUE &R, int p);
        int EdgeInsert(node_t x, edge_t y,PRIORITY_Q &PQ, QUEUE &R,
                vector<node_t> &Vblack, vector<node_t> &Vcolor, int p); // insert to double-linked list
        ...
        int EdgeRemove(node_t x, edge_t y, QUEUE &R, vector<node_t> &Vblack, 
                vector<bool> &A, vector<node_t> &groups); //remove in double-linked list.      
